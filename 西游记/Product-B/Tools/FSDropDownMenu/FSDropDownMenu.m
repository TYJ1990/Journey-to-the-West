...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        //tableView init
        _leftTableView = [[UITableView alloc] initWithFrame:CGRectMake(origin.x, self.frame.origin.y + self.frame.size.height, kScreenWidth*0.7, 0) style:UITableViewStylePlain];
        _leftTableView.rowHeight = 38;
        _leftTableView.dataSource = self;
        _leftTableView.delegate = self;
        
        
        _rightTableView = [[UITableView alloc] initWithFrame:CGRectMake(origin.x+kScreenWidth*0.7, self.frame.origin.y + self.frame.size.height, kScreenWidth*0.3, 0) style:UITableViewStylePlain];
...
- (void)animateTableViewShow:(BOOL)show complete:(void(^)())complete {
    if (show) {
        ...
        _leftTableView.frame = CGRectMake(self.origin.x, self.frame.origin.y, kScreenWidth*0.7, 0);
        [self.superview addSubview:_leftTableView];
        _rightTableView.frame = CGRectMake(self.origin.x+kScreenWidth*0.7, self.frame.origin.y, kScreenWidth*0.3, 0);
        [self.superview addSubview:_rightTableView];
        
        _leftTableView.alpha = 1.f;
        _rightTableView.alpha = 1.f;
        [UIView animateWithDuration:0.2 animations:^{
            _leftTableView.frame = CGRectMake(self.origin.x, self.frame.origin.y, kScreenWidth*0.7, _height);
            _rightTableView.frame = CGRectMake(self.origin.x+kScreenWidth*0.7, self.frame.origin.y, kScreenWidth*0.3, _height);

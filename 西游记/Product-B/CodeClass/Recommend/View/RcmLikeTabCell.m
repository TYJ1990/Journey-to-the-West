...
- (void)cellCongigureWithModel:(CellModel *)model
{
    [self.recImageView sd_setImageWithURL:[NSURL URLWithString:model.photo] placeholderImage:kPlacehodeImage completed:nil];
    self.recDetailLabel.text = model.title;
    self.recUserNameLabel.text = model.username;
    NSNumber *numLift = model.views;
    NSString* liftStr=[NSString stringWithFormat:@"浏览量 %@",numLift];
    self.recLiftLabel.text=liftStr;
    NSString* rightStr = model.replys;
    self.recRightLabel.text=[NSString stringWithFormat:@"回复量 %@",rightStr];
}
@end
